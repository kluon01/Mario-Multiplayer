...
class Server
{
	...
	static int mx = 0;
	static double my = 0;
	static double mv_vel = 0;
	static int lx = 0;
	static double ly = 0;
	static double lv_vel = 0;
	...
	static void onPost(OutputStream os, String url, char[] incomingPayload)
	{
		...
		int client_ID = (int)incoming.getLong("ID");
		// Make a response
		if(client_ID > 2)
		{
			throw new IllegalArgumentException("Client ID is greater than number of windows, please restart.");
		}
		if(client_ID < 0)
		{
			client_Count++;
			outgoing.add("ID" , client_Count);
			mx = (int)incoming.getLong("mx");
			my = incoming.getDouble("my");
			mv_vel = incoming.getDouble("mv_vel");
			lx = (int)incoming.getLong("lx");
			ly = incoming.getDouble("ly");
			lv_vel = incoming.getDouble("lv_vel");
			outgoing.add("mx" , mx);
			outgoing.add("my" , my);
			outgoing.add("mv_vel", mv_vel);
			outgoing.add("lx" , lx);
			outgoing.add("ly" , ly);
			outgoing.add("lv_vel", lv_vel);
		}
		else if(client_ID == 1)
		{
			mx = (int) incoming.getLong("mx");
	...
	public static void main(String[] args) throws Exception
	{
		...
		int port = 12345;
		String ServerURL = "http://localhost:" + Integer.toString(port) + "/mario.html";
